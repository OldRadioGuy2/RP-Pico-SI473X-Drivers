...
int16_t SI4735::getDeviceI2CAddress(uint8_t resetPin)
{
    ...
    /*
    * RP Pico: Standard C++ does not support this
    * Serial.println("Error was:" + String(error));
    */
    //Serial.println("Error was:" + to_string(error));
...
void SI4735::waitToSend()
{
    ...
    do
    {
        ...
        result= Wire.requestFrom(deviceAddress, 1);
        if (result > 0) {
            status = Wire.read();
        }
    } while (!(status & B10000000));
    return;
}
...
#ifdef RP_PICO        
        attachInterrupt(digitalPinToInterrupt(interruptPin), interrupt_hundler, IRQ_RISING);
#else        
...
{
    setup(resetPin, -1, defaultFunction, SI473X_ANALOG_AUDIO, XOSCEN_RCLK);
    delay(250);
}
...
void SI4735::setBandwidth(uint8_t AMCHFLT, uint8_t AMPLFLT)
{
    ...
    Wire.write((uint8_t) 0x00);                  // Always 0x00
...
void SI4735::setRdsConfig(uint8_t RDSEN, uint8_t BLETHA, uint8_t BLETHB, uint8_t BLETHC, uint8_t BLETHD)
{
    ...
    Wire.beginTransmission(deviceAddress);
    Wire.write(SET_PROPERTY);
    Wire.write((uint8_t) 0x00);        // Always 0x00 (I need to check it)
    Wire.write(property.raw.byteHigh); // Send property - High byte - most significant first
...
si4735_eeprom_patch_header SI4735::downloadPatchFromEeprom(int eeprom_i2c_address)
{
    ...
    Wire.write((uint8_t) 0x00); // offset Most significant Byte
    Wire.write((uint8_t) 0x00); // offset Less significant Byte
    Wire.endTransmission();
    delay(5);
...
void SI4735::setFrequencyNBFM(uint16_t freq)
{
    ...
    Wire.beginTransmission(deviceAddress);
    Wire.write(0x50);
    Wire.write((uint8_t) 0x00); // Send a byte with FAST and  FREEZE information; if not FM must be 0;
